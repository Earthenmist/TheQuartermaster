...
-- Legacy settings remain stored while the shared appearance is fixed.
...
function TheQuartermaster:OnInitialize()
    ...
    self:ReconcileCurrentCharacterIdentity()
    self:PreserveUnscopedGuildBanks()
...
function TheQuartermaster:OnEnable()
    ...
    if self.InitializeWealth then self:InitializeWealth() end
    if self.InitializeAuctionTracking then self:InitializeAuctionTracking() end
    if self.InitializeProfessionDepth then self:InitializeProfessionDepth() end
    if self.InitializeKnowledge then self:InitializeKnowledge() end
    if self.InitializeQuestLogs then self:InitializeQuestLogs() end
    if self.InitializeAchievements then self:InitializeAchievements() end
    ...
    self:RegisterEvent("QUEST_TURNED_IN", "OnWeeklyQuestChanged")
    self:RegisterEvent("QUEST_DATA_LOAD_RESULT", "OnWeeklyQuestChanged")
    self:RegisterEvent("UPDATE_UI_WIDGET", "OnWeeklyWidgetChanged")
...
function TheQuartermaster:OnDisable()
    if self.StopWealth then self:StopWealth() end
    if self.StopAuctionTracking then self:StopAuctionTracking() end
    if self.StopProfessionDepth then self:StopProfessionDepth() end
    if self.StopKnowledge then self:StopKnowledge() end
    if self.StopQuestLogs then self:StopQuestLogs() end
    if self.StopAchievements then self:StopAchievements() end
    if self.CancelMailAttachmentScan then self:CancelMailAttachmentScan() end
    if self.StopMailExpiry then self:StopMailExpiry() end
    if self._mailFrame then self._mailFrame:UnregisterAllEvents() end
    self._mailTrackingInitialized = nil
    self._mailboxOpen = false
    if self.OnRecipeProfessionClosed then self:OnRecipeProfessionClosed() end
...
function TheQuartermaster:SlashCommand(input)
    ...
    elseif cmd == "cleanup" then
        self:Print(L["CHARACTER_RETENTION_NOTICE"])
        return
    elseif cmd == "resetrep" then
        ...
        return
    elseif cmd == "pveperf" then
        local stats = self.GetPvEResourceStats and self:GetPvEResourceStats()
        if stats then
            self:Print(string.format(L["PVE_PERF_STATS"], stats.frames, stats.textures,
                stats.fontStrings, stats.activeTickers, stats.passes, stats.lastBuildMS))
        else
            self:Print(L["PVE_PERF_NOT_BUILT"])
        end
        return
    ...
    elseif cmd == "recipescan" then
        if self.PrintRecipeScanDiagnostic then self:PrintRecipeScanDiagnostic() end
    ...
    if self.ScanGuildBank then
        ...
        C_Timer.After(0.5, function()
            ...
            if n and n > 0 and QueryGuildBankTab then
                if TheQuartermaster.guildBankIsOpen then
...
function TheQuartermaster:OnCurrencyChanged()
    if self.OnWeeklyQuestChanged then self:OnWeeklyQuestChanged() end
...
function TheQuartermaster:OnBagUpdate(eventOrBagIDs, ...)
    ...
    elseif type(eventOrBagIDs) == "string" then
        ...
        elseif event == "GUILDBANKBAGSLOTS_CHANGED" then
            if self.guildBankIsOpen and GetCurrentGuildBankTab then
                self.guildBankReadyTabs = self.guildBankReadyTabs or {}
                self.guildBankReadyTabs[GetCurrentGuildBankTab()] = true
            end
