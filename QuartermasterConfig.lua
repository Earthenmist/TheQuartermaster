...
local options = {
    ...
    type = "group",
    args = {
        allowWealthTransactionRemoval = {
            order = 790, type = "toggle", width = "full",
            name = L.WL_REMOVAL_SETTING, desc = L.WL_REMOVAL_SETTING_DESC,
            get = function() return TheQuartermaster.db.profile.allowWealthTransactionRemoval == true end,
            set = function(_, value)
                TheQuartermaster.db.profile.allowWealthTransactionRemoval = value == true
                TheQuartermaster.wealthRemoveID = nil
                if TheQuartermaster.RefreshUI then TheQuartermaster:RefreshUI() end
            end,
        },
        -- Header
        header = {
        ...
        currentLanguageInfo = {
            ...
            name = function()
                ...
                return string.format(L.CFG_LANGUAGE_INFO, localeName)
            end,
            fontSize = "medium",
        },
        ...
        autoOptimize = {
            ...
            desc = L["SAFE_DATABASE_MAINTENANCE_DESCRIPTION"],
            width = 1.5,
        ...
        windowScale = {
            order = 43.5,
            type = "range",
            name = L.CFG_WINDOW_SCALE,
            desc = L.CFG_WINDOW_SCALE_DESC,
            min = 0.70,
            max = 1.15,
            step = 0.05,
            isPercent = true,
        ...
        notificationsEnabled = {
            ...
            set = function(_, value)
                TheQuartermaster.db.profile.notifications.enabled = value
                TheQuartermaster:CheckMailExpiry()
            end,
        },
        showUpdateNotes = {
            order = 73,
        ...
        showMailExpiry = {
            order = 75.5,
            type = "toggle",
            name = L.MAIL_EXPIRY_TITLE,
            desc = L.MAIL_EXPIRY_SETTING,
            width = 1.5,
            disabled = function() return not TheQuartermaster.db.profile.notifications.enabled end,
            get = function() return TheQuartermaster.db.profile.notifications.showMailExpiry ~= false end,
            set = function(_, value)
                TheQuartermaster.db.profile.notifications.showMailExpiry = value
                TheQuartermaster:CheckMailExpiry()
            end,
        },
        resetVersionButton = {
...
-- Reuse the existing callbacks and saved keys in focused native Settings pages.
-- Blizzard owns the surrounding Settings controls; do not skin shared widgets.
do
    local original = options.args
    local groups = {
        {"general", "CFG_GENERAL", "CFG_GENERAL_DESC", {"enabled", "minimapIcon", "suppressPlayedTimeChat"}},
        {"collection", "CFG_COLLECTION", "CFG_COLLECTION_DESC", {"autoScan", "trackProfessionRecipes", "autoSaveChanges", "tabHeader", "tabDesc", "ignoredTab1", "ignoredTab2", "ignoredTab3", "ignoredTab4", "ignoredTab5"}},
        {"display", "DISPLAY", "CFG_DISPLAY_DESC", {"windowScale", "showItemLevel", "showItemCount", "tooltipHeader", "tooltipEnhancement", "tooltipGuildCounts", "currencyHeader", "currencyFilterMode", "currencyShowZero"}},
        {"notifications", "NOTIFICATIONS", "CFG_NOTIFICATIONS_DESC", {"notificationsEnabled", "showUpdateNotes", "showVaultReminder", "showLootNotifications", "showMailExpiry", "resetVersionButton"}},
        {"data", "CFG_DATA", "CFG_DATA_DESC", {"allowWealthTransactionRemoval", "autoOptimize", "databaseStatsButton", "optimizeDatabaseButton", "characterManagementHeader", "characterManagementDesc", "deleteCharacterDropdown", "deleteCharacterButton", "wipeAllData"}},
        {"help", "CFG_HELP", "CFG_HELP_DESC", {"currentLanguageInfo", "commandsHeader", "commandsDesc", "debugMode"}},
    }
    original.autoScan.name, original.autoScan.desc = L.CFG_BANK_SCAN, L.CFG_BANK_SCAN_DESC
    original.trackProfessionRecipes.desc = L.CFG_RECIPE_SCAN_DESC
    original.autoSaveChanges.disabled = true
    original.autoSaveChanges.name, original.autoSaveChanges.desc = L.CFG_LEGACY_SYNC, L.CFG_LEGACY_SYNC_DESC
    original.commandsDesc.name = L.CFG_COMMANDS
    original.currentLanguageInfo.fontSize = "medium"
    local visibleHelp = {autoScan = "CFG_BANK_SCAN_DESC", trackProfessionRecipes = "CFG_RECIPE_SCAN_DESC",
        autoSaveChanges = "CFG_LEGACY_SYNC_DESC", showMailExpiry = "CFG_MAIL_REMINDERS", tooltipGuildCounts = "CFG_GUILD_TOOLTIP_DESC"}
    options.args, options.childGroups = {}, "tree"
    for order, group in ipairs(groups) do
        local args = {intro = {type = "description", name = L[group[3]], order = 0, fontSize = "medium", width = "full"}}
        for index, key in ipairs(group[4]) do
            local option = original[key]
            option.order = index * 2
            if option.type == "toggle" or option.type == "select" or option.type == "execute" or option.type == "color" then
                option.width = "full"
            end
            args[key] = option
            if visibleHelp[key] then
                args[key .. "Help"] = {type = "description", name = L[visibleHelp[key]], order = index * 2 + 1, width = "full"}
            end
        end
        options.args[group[1]] = {type = "group", name = L[group[2]], order = order, args = args}
    end
    options.args.collection.args.sources = {type = "description", name = L.CFG_SOURCE_GUIDE, order = 99, width = "full"}
end
