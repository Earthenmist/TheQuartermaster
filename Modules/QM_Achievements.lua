...
function Addon:ScanAchievements()
    ...
    local function Step()
        ...
        local clock=GetTimePreciseSec or GetTime or time
        local started=clock()
        for step=1,25 do
            if step>1 and clock()-started>=0.002 then break end
            if ci<=#cats then
                if count==nil then
...
function Addon:InitializeAchievements()
    ...
    for _,event in ipairs({"PLAYER_LEAVING_WORLD","PLAYER_REGEN_DISABLED","PLAYER_LOGOUT"}) do f:RegisterEvent(event) end
    f:SetScript("OnEvent",function(_,event)
        if event=="PLAYER_LOGOUT" then self:StopAchievements()
        else self._achievementJob=nil;self._achievementPending=nil;self.achievementStatus="paused";Refresh(self) end
    end)
    -- Full collection is explicitly requested from the achievement page.
end
