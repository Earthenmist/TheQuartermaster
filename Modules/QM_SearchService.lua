...
-- opts.deferRefresh: caller owns the final redraw for a batch of target changes
function TheQuartermaster:ToggleWatchlistReagent(itemID, opts)
    ...
    if shouldToggle then
        if entry then
            wl.reagents[itemID] = nil
            if not opts.deferRefresh and self.RefreshUI then self:RefreshUI() end
            return
        else
            entry = {}
...
function TheQuartermaster:CountItemTotals(itemID, includeGuildBank)
    ...
    -- Mail awaiting collection; COD attachments require payment and are not stock.
    if self.GetMailItemRows then
        for _, row in ipairs(self:GetMailItemRows()) do
            if row.item.itemID == itemID and not row.cod then
                Add(row.characterKey .. " - " .. ns.L.MAIL_SOURCE, row.item.stackCount)
            end
        end
    end
...
-- Item filters combine recorded values with resolved base metadata from the
-- shared item API. Instance item levels are never borrowed by item ID.
local F = {}
ns.SearchFilters = F
local fields = {"slot", "quality", "expansion", "minItemLevel", "maxItemLevel", "minRequiredLevel", "maxRequiredLevel"}
...
function F.Normalize(input)
    local result = {}
    input = type(input) == "table" and input or {}
    result.slot = Public(input.slot, "string")
    if result.slot == "" then result.slot = nil end
    for _, key in ipairs(fields) do
        if key ~= "slot" then result[key] = Number(input[key]) end
    end
    return result
end
function F.Key(input)
    local values = {}
    local f = F.Normalize(input)
    for _, key in ipairs(fields) do values[#values + 1] = tostring(f[key] or "") end
    return table.concat(values, ":")
end
function F.Active(input)
    return next(F.Normalize(input)) ~= nil
end
function F.Metadata(addon, item, cache)
    local link = Public(item.itemLink, "string")
    local id = Number(item.itemID)
    local key = link or id
    local info = key and cache[key]
    if not info then
        info = {}
        if key and addon.API_GetItemInfo then
            local values = {pcall(addon.API_GetItemInfo, addon, key)}
            if values[1] then
                info.quality = Number(values[4])
                info.requiredLevel = Number(values[6])
                info.slot = Public(values[10], "string")
                info.expansion = Number(values[16])
                -- GetItemInfo's base level is not an instance's effective item level.
            end
        end
        if link and link:find("|Hitem:", 1, true) and C_Item and C_Item.GetDetailedItemLevelInfo then
            local ok, level = pcall(C_Item.GetDetailedItemLevelInfo, link)
            if ok then info.itemLevel = Number(level) end
        end
        if key then cache[key] = info end
    end
    return {
        quality = info.quality or Number(item.quality),
        slot = Public(item.equipLoc, "string") or info.slot,
        expansion = Number(item.expansionID) or Number(item.expacID) or info.expansion,
        requiredLevel = Number(item.minLevel) or info.requiredLevel,
        itemLevel = Number(item.itemLevel) or info.itemLevel,
    }
end
function F.Match(metadata, input)
    local unknown = false
    for _, key in ipairs({"slot", "quality", "expansion"}) do
        local wanted = input[key]
        if wanted ~= nil then
            local actual = metadata[key]
            if actual == nil then unknown = true
            elseif not (actual == wanted or (wanted == "INVTYPE_CHEST" and actual == "INVTYPE_ROBE")) then return false, false end
        end
    end
    for _, group in ipairs({{"itemLevel", "minItemLevel", "maxItemLevel"}, {"requiredLevel", "minRequiredLevel", "maxRequiredLevel"}}) do
        local low, high, value = input[group[2]], input[group[3]], metadata[group[1]]
        if low or high then
            if value == nil then unknown = true
            elseif (low and value < low) or (high and value > high) then return false, false end
        end
    end
    return not unknown, unknown
end

function TheQuartermaster:PerformGlobalSearch(searchText, mode, includeGuildBank, filters)
    local text = tostring(searchText or ""):gsub("^%s+", ""):gsub("%s+$", "")
    filters = F.Normalize(filters)
    local active = F.Active(filters)
    if text == "" and (not active or mode == "currency") then
        return { items = {}, currencies = {}, unknownItems = 0 }
    end

    mode = mode or "all"
    if includeGuildBank == nil then
        local wl = GetProfileWatchlist(self)
        includeGuildBank = not wl or wl.includeGuildBank ~= false
    end

    local out = { items = {}, currencies = {}, unknownItems = 0 }

    if mode == "all" or mode == "items" or mode == "reagents" then
        local itemResults = (self.PerformItemSearch and self:PerformItemSearch(text, text == "" and active)) or {}
        ...
        if active then
            ...
            for _, row in ipairs(itemResults) do
                local matches, unknown = F.Match(F.Metadata(self, row.item or {}, cache), filters)
                if matches then filtered[#filtered + 1] = row end
                if unknown then out.unknownItems = out.unknownItems + 1 end
            end
            itemResults = filtered
        end
    ...
    if mode == "currency" or (mode == "all" and not active) then
