--[[
    The Quartermaster - Error Handler Module
...
function TheQuartermaster:SafeCall(func, context, ...)
...
function TheQuartermaster:SafeCallSilent(func, context, ...)
...
function TheQuartermaster:WrapFunction(func, context)
    return function(...)
...
function TheQuartermaster:LogError(message, context, stack)
...
function TheQuartermaster:GetErrorStats()
    return {
...
function TheQuartermaster:GetRecentErrors(count)
    count = count or 10
    local recent = {}
...
function TheQuartermaster:ClearErrorLog()
    errorLog = {}
    errorStats = {
        total = 0,
        byFunction = {},
...
function TheQuartermaster:PrintErrorStats()
...
function TheQuartermaster:PrintRecentErrors(count)
    count = count or 5
...
function TheQuartermaster:ShowErrorDetails(errorIndex)
...
function TheQuartermaster:WrapCriticalFunctions()
...
function TheQuartermaster:EmergencyRecovery()
    ...
    C_Timer.After(1, function()
        if TheQuartermaster then
            TheQuartermaster:Print("|cff00ff00Recovery complete.|r Try opening the window again (/wn show)")
        end
    end)
end
...
function TheQuartermaster:ExportErrorLog()
    if #errorLog == 0 then
        return "No errors recorded"
    end
    
    local export = {}
    table.insert(export, "The Quartermaster Error Log")
...
function TheQuartermaster:InitializeErrorHandler()
