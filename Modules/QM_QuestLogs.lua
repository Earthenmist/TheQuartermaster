...
function Addon:ScanQuestLogs()
    ...
    local function Step()
        ...
        for i=job.index,math.min(#list,job.index) do
            local row=list[i]
...
function Addon:InitializeQuestLogs()
    ...
    for _,event in ipairs({"PLAYER_ENTERING_WORLD","PLAYER_LEAVING_WORLD","PLAYER_LOGOUT","PLAYER_REGEN_DISABLED"}) do frame:RegisterEvent(event) end
    frame:SetScript("OnEvent",function(_,event)
        if self._questLogsReading then return end
        if event~="PLAYER_ENTERING_WORLD" and event~="PLAYER_LEAVING_WORLD" and event~="PLAYER_LOGOUT" and event~="PLAYER_REGEN_DISABLED" then return end
    ...
    -- Full collection is explicitly requested from the quest-log page.
end
