...
local function DrawPersonalBankSlotView(self, parent, yOffset, width, itemsSearchText)
    ...
    for displayIndex = 1, visibleCount do
        ...
        local label = btn:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
...
local function DrawGuildBankSlotView(self, parent, yOffset, width, itemsSearchText)
    ...
    local guildName = self:GetGuildIdentity()
    ...
    if maxTab <= 0 then
        local msg = parent:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
        msg:SetPoint("TOPLEFT", 18, -yOffset)
        msg:SetWidth(width - 36)
        msg:SetWordWrap(true)
        msg:SetTextColor(0.7, 0.7, 0.7)
        msg:SetText(L[self.db.global.legacyGuildBanks and next(self.db.global.legacyGuildBanks) and "LEGACY_GUILD_CACHE_NOTICE" or "NO_GUILD_BANK_DATA_CACHED_YET_OPEN_YOUR_GUILD_BANK"])
        return yOffset + math.max(30, msg:GetStringHeight() + 16)
    end
...
local function ResetExpandedGroups()
    ...
    local invText = inventoryBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    local personalText = personalBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    local warbandText = warbandBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    if ENABLE_GUILD_BANK then
        ...
        local guildText = guildBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    local function CreateStableToggleButton(parent, label)
        ...
        btn.text = btn:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
    ...
    if currentItemsSubTab == "warband" then
        ...
        local function ApplyThemedActionButton(btn)
            ...
            btn:SetNormalFontObject("QuartermasterFontSmall")
            btn:SetHighlightFontObject("QuartermasterFontSmall")
        ...
        local goldDisplay = tabFrame:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
        ...
        toggleBtn.text = toggleBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
        ...
        toggleBtn:SetBackdropBorderColor(COLORS.border[1], COLORS.border[2], COLORS.border[3], 1)
...
-- Personal Bank has no gold controls (WoW doesn't support gold storage in personal bank)
    ...
    if currentItemsSubTab == "guild" then
        ...
        local guildGoldDisplay = tabFrame:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
        ...
        local mode = (self.db and self.db.profile and self.db.profile.guildBankViewMode) or "list"
        local toggleBtn = CreateFrame("Button", nil, tabFrame, "BackdropTemplate")
        toggleBtn:SetSize(96, 24)
        toggleBtn:SetPoint("RIGHT", tabFrame, "RIGHT", -5, 0)
        toggleBtn:SetBackdrop({
        ...
        toggleBtn:SetScript("OnClick", function()
            local cur = (self.db.profile.guildBankViewMode or "list")
        ...
        toggleBtn:HookScript("OnShow", function(btn)
            local m = (self.db.profile.guildBankViewMode or "list")
            if btn.text then
                btn.text:SetText(m == "slots" and "List View" or "Slot View")
            end
        end)
        ...
        local function DrawInventorySlots(startY)
            ...
            for i = 1, bagCount do
                ...
                if bagID ~= 5 or (bagSizes[i] and bagSizes[i] > 0) then
                    ...
                    local t = btn:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
                    t:SetPoint("CENTER")
            ...
            if numSlots <= 0 then
                local msg = grid:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
            ...
            for slot = 1, numSlots do
                ...
                local countText = btn:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
...
btn:SetScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    local statsText = statsBar:CreateFontString(nil, "OVERLAY", "QuartermasterFontSmall")
    ...
    if currentItemsSubTab == "guild" then
        local viewMode = (self.db and self.db.profile and self.db.profile.guildBankViewMode) or "list"
        if viewMode == "slots" then
    ...
    for _, item in ipairs(items) do
        ...
        if not groups[typeName] then
            -- Categories start collapsed and only one sibling can open.
    ...
    for _, typeName in ipairs(groupOrder) do
        ...
        local function ToggleGroup(key, isExpanded)
            local open = isExpanded
            if open == nil then open = not expandedGroups[key] end
            ResetExpandedGroups()
            if open then expandedGroups[key] = true end
            TheQuartermaster:RefreshUI()
        end
        
        -- Create collapsible header with a neutral border and icon
