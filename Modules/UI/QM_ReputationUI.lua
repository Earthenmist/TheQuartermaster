--[[
    The Quartermaster - Reputation Tab
...
local function CreateReputationRow(parent, reputation, factionID, rowIndex, indent, width, yOffset, subfactions, IsExpanded, ToggleExpand, characterInfo)
    ...
    if standingWord ~= "" then
        ...
        local standingText = ns.UI_RenderFontString(row, nil, "OVERLAY", "QuartermasterFontBody")
        ...
        local numberText = ns.UI_RenderFontString(row, nil, "OVERLAY", "QuartermasterFontBody")
        ...
        local separator = ns.UI_RenderFontString(row, nil, "OVERLAY", "QuartermasterFontBody")
    ...
    if characterInfo then
        local badgeText = ns.UI_RenderFontString(row, nil, "OVERLAY", "QuartermasterFontSmall")
    ...
    local progressBorder = ns.UI_RenderFrame("Frame", nil, row, "BackdropTemplate")
    ...
    local progressBg = ns.UI_RenderTexture(row, nil, "BACKGROUND")
    ...
    if isParagon then
        ...
        local paragonFrame = ns.UI_RenderFrame("Frame", nil, row)
        paragonFrame:SetSize(18, 18)
        paragonFrame:SetPoint("RIGHT", progressBorder, "LEFT", -24, 0)
        
        local paragonIcon = ns.UI_RenderTexture(paragonFrame, nil, "OVERLAY")
    ...
    if baseReputationMaxed then
        -- Create frame for tooltip support
        local checkFrame = ns.UI_RenderFrame("Frame", nil, row)
        checkFrame:SetSize(16, 16)
        checkFrame:SetPoint("RIGHT", progressBorder, "LEFT", -4, 0)
        
        local checkmark = ns.UI_RenderTexture(checkFrame, nil, "OVERLAY")
    ...
    if currentValue > 0 or baseReputationMaxed then
        local progressBar = ns.UI_RenderTexture(row, nil, "ARTWORK")
    ...
    -- Fixed-size outlined text stays legible across bright and dark fills.
    local progressText = ns.UI_RenderFontString(row, nil, "OVERLAY", "QuartermasterFontEmphasis")
    progressText:SetPoint("CENTER", progressBg, "CENTER", 0, 0)
    progressText:SetSize(progressBarWidth - 12, progressBarHeight)
    progressText:SetJustifyH("CENTER")
    progressText:SetJustifyV("MIDDLE")
    progressText:SetWordWrap(false)
    progressText:SetShadowOffset(0, 0)
...
local function DrawContent(self, parent)
    ...
    if not C_Reputation or not C_Reputation.GetNumFactions then
        ...
        local errorFrame = ns.UI_RenderFrame("Frame", nil, parent, "BackdropTemplate")
        ...
        local errorIcon = ns.UI_RenderTexture(errorFrame, nil, "ARTWORK")
        errorIcon:SetSize(32, 32)
        errorIcon:SetPoint("LEFT", 15, 0)
        errorIcon:SetTexture("Interface\\DialogFrame\\UI-Dialog-Icon-AlertNew")
        
        local errorText = ns.UI_RenderFontString(errorFrame, nil, "OVERLAY", "QuartermasterFontBody")
        ...
        local errorDesc = ns.UI_RenderFontString(errorFrame, nil, "OVERLAY", "QuartermasterFontSmall")
    ...
    local IsExpanded, ToggleExpand = ns.UI_ProgressionAccordion("reputations", function() self:RefreshUI() end)
    ...
    charactersWithReputations = ns.SortCharacterRows(self.db, charactersWithReputations, function(row) return row.key end)
    
    if not hasAnyData then
        DrawEmptyState(parent, 
            reputationSearchText ~= "" and L.PG_REPUTATION_SEARCH_EMPTY or L.PG_NO_REPUTATIONS,
            yOffset)
        return yOffset + 100
    end
    ...
    if viewMode == "filtered" then
        ...
        local awSectionExpanded = IsExpanded(awSectionKey, false, 1)  -- Default collapsed
        ...
        awSectionHeader:SetBackdropColor(unpack(COLORS.bgCard))
        local COLORS = GetCOLORS()
        awSectionHeader:SetBackdropBorderColor(unpack(COLORS.border))
        ...
        if awSectionExpanded then
            if totalAccountWide == 0 then
                -- Empty state
                local emptyText = ns.UI_RenderFontString(parent, nil, "OVERLAY", "QuartermasterFontBody")
        ...
        for _, headerData in ipairs(accountWideHeaders) do
            ...
            local headerExpanded = IsExpanded(headerKey, false, 2)
        ...
        local cbSectionExpanded = IsExpanded(cbSectionKey, false, 1)  -- Default collapsed
        ...
        cbSectionHeader:SetBackdropColor(unpack(COLORS.bgCard))
        cbSectionHeader:SetBackdropBorderColor(unpack(COLORS.border))
...
function TheQuartermaster:DrawReputationTab(parent)
    if not parent then return 0 end
    self.reputationRenderPool = self.reputationRenderPool or ns.UI_NewRenderPool(parent)
    local pool = self.reputationRenderPool
