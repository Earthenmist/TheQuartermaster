...
-- Only complete snapshots replace persistent data. Failure details stay generic so
-- inaccessible API values are never formatted or stored in diagnostics.
local function CopyFields(source)
    local copy = {}
    for k, v in pairs(source or {}) do copy[k] = v end
    return copy
end

local function ValidateContainer(self, bagID, numSlots, observed, previousSize)
    assert(type(numSlots) == "number" and numSlots >= 0, "slots unavailable")
    assert(not previousSize or previousSize == 0 or numSlots > 0, "bag not ready")
    if numSlots == 0 then return end
    local free = self:API_GetFreeBagSlots(bagID)
    assert(type(free) == "number" and free >= 0 and free <= numSlots, "free slots unavailable")
    assert(observed == numSlots - free, "partial container data")
end

local function RunSnapshot(self, domain, build, commit)
    local ok, snapshot = pcall(build)
    self.scanStatus = self.scanStatus or {}
    if not ok or type(snapshot) ~= "table" then
        self.scanStatus[domain] = "unavailable"
        self.lastScanSuccess = false
        if self.UpdateFooter then self:UpdateFooter() end
        return false
    end
    commit(snapshot)
    self.scanStatus[domain] = "fresh"
    self.lastScanSuccess = true
    self.lastScanTime = time()
    if self.InvalidateItemCache then self:InvalidateItemCache() end
    if self.InvalidateTooltipCache then self:InvalidateTooltipCache() end
    return true
end
...
function TheQuartermaster:ScanWarbandBank()
    local success = RunSnapshot(self, "warband", function()
        assert(self:IsWarbandBankOpen(), "bank closed")
        local previous = self.db.global.warbandBank or {}
        local snapshot = CopyFields(previous)
        snapshot.items, snapshot.tabSizes = {}, {}
        local totalItems = 0
        local totalSlots = 0
        local usedSlots = 0
        ...
        for tabIndex, bagID in ipairs(ns.WARBAND_BAGS) do
            ...
            for slotID = 1, numSlots do
                ...
                if itemInfo and itemInfo.itemID then
                    ...
                    snapshot.items[tabIndex][slotID] = {
            ...
            ValidateContainer(self, bagID, numSlots, occupied, previous.tabSizes and previous.tabSizes[tabIndex])
        end
        assert(totalSlots > 0, "bank not ready")

        -- Update metadata
        snapshot.lastScan = time()
        snapshot.totalSlots = totalSlots
        snapshot.usedSlots = usedSlots

        -- Get Warband bank gold
        if C_Bank and C_Bank.FetchDepositedMoney then
            snapshot.gold = C_Bank.FetchDepositedMoney(Enum.BankType.Account) or previous.gold
        end


        return snapshot
    end, function(snapshot) self.db.global.warbandBank = snapshot end)
    if success and self.RefreshUI then self:RefreshUI() end
    return success
end
...
function TheQuartermaster:ScanPersonalBank()
    local success = RunSnapshot(self, "personal", function()
        assert(self.bankIsOpen, "bank closed")
        local previous = self.db.char.personalBank or {}
        local snapshot = CopyFields(previous)
        snapshot.items, snapshot.bagSizes, snapshot.bagIDs = {}, {}, {}
        local totalItems = 0
        local totalSlots = 0
        local usedSlots = 0
        ...
        for bagIndex, bagID in ipairs(ns.PERSONAL_BANK_BAGS) do
            snapshot.items[bagIndex] = {}
            snapshot.bagIDs[bagIndex] = bagID

            -- Use API wrapper (TWW compatible)
            local numSlots = self:API_GetBagSize(bagID)
            snapshot.bagSizes[bagIndex] = numSlots
            totalSlots = totalSlots + numSlots
            ...
            for slotID = 1, numSlots do
                ...
                if itemInfo and itemInfo.itemID then
                    ...
                    snapshot.items[bagIndex][slotID] = {
                        itemID = itemInfo.itemID,
                        itemLink = itemInfo.hyperlink,
                        stackCount = itemInfo.stackCount or 1,
            ...
            ValidateContainer(self, bagID, numSlots, bagItemCount, previous.bagSizes and previous.bagSizes[bagIndex])
        end
        assert(totalSlots > 0, "bank not ready")
        ...
        return snapshot
    end, function(snapshot) self.db.char.personalBank = snapshot end)
    if success then
        if self.SaveCurrentCharacterData then self:SaveCurrentCharacterData() end
...
function TheQuartermaster:ScanGuildBank()
    local guildName, guildKey, guildRealm
    local success = RunSnapshot(self, "guild", function()
        assert(self.guildBankIsOpen and IsInGuild(), "guild bank closed")
        guildKey, guildName, guildRealm = self:GetGuildIdentity()
        assert(guildKey, "guild unavailable")
        local previous = self.db.global.guildBank and self.db.global.guildBank[guildKey] or {}
        local guildData = CopyFields(previous)
        guildData.name, guildData.realm, guildData.identityVersion = guildName, guildRealm, 1
        guildData.tabs = CopyFields(previous.tabs)
        local scannedTabs = 0
        -- Cache guild bank money (copper). This is display-only and is NOT included in totals.
        if type(GetGuildBankMoney) == "function" then
            guildData.money = GetGuildBankMoney() or previous.money
        end
        ...
        -- Refresh only the selected tab after its contents event.
        for tabIndex = 1, numTabs do
            ...
            if isViewable and tabIndex == GetCurrentGuildBankTab()
                and self.guildBankReadyTabs and self.guildBankReadyTabs[tabIndex] then
                scannedTabs = scannedTabs + 1
                guildData.tabs[tabIndex] = { name = name, icon = icon, items = {}, lastScan = time() }
                ...
                for slotID = 1, MAX_GUILDBANK_SLOTS_PER_TAB do
                    ...
                    assert(itemLink or (not texture and (not itemCount or itemCount == 0)), "guild item not ready")
                    if itemLink then

                        -- Extract itemID from link
                        local itemID = tonumber(itemLink:match("item:(%d+)"))

                        assert(itemID, "guild item ID unavailable")
                        if itemID then
                            usedSlots = usedSlots + 1
                            ...
                            if (not itemQuality) and itemLink then
                                local color = itemLink:match("|c%x%x(%x%x%x%x%x%x)%x%x%x%x") or itemLink:match("|c%x%x%x%x(%x%x%x%x%x%x)")
                                if color then
                                    color = color:lower()
                                    local qmap = { ["9d9d9d"]=0, ["ffffff"]=1, ["1eff00"]=2, ["0070dd"]=3, ["a335ee"]=4, ["ff8000"]=5, ["e6cc80"]=6, ["00ccff"]=7 }
                                    itemQuality = qmap[color]
        ...
        assert(scannedTabs > 0, "guild tab not ready")
        -- Recount retained tabs too; their individual timestamps remain unchanged.
        totalItems, totalSlots, usedSlots = 0, 0, 0
        for _, tab in pairs(guildData.tabs) do
            totalSlots = totalSlots + 98
            for _, item in pairs(tab.items or {}) do
                usedSlots = usedSlots + 1
                totalItems = totalItems + (item.stackCount or 1)
            end
        end
        -- Update metadata
        ...
        self.db.global.guildBank = self.db.global.guildBank or {}
        self.db.global.guildBank[guildKey] = snapshot
    end)
    if success and self.RefreshUI then self:RefreshUI() end
    return success
end
...
function TheQuartermaster:GetGuildBankItems(groupByCategory)
    local items = {}
    local guildKey = self:GetGuildIdentity()
    
    if not guildKey or not self.db.global.guildBank or not self.db.global.guildBank[guildKey] then
        return items
    end
    
    local guildData = self.db.global.guildBank[guildKey]
...
-- Guild Bank stats
    local guildKey = self:GetGuildIdentity()
    if guildKey and self.db.global.guildBank and self.db.global.guildBank[guildKey] then
        local guildData = self.db.global.guildBank[guildKey]
    ...
    for i = 1, numFactions do
        ...
        if factionData and factionData.name then
            ...
            elseif factionData.factionID and not factionData.isHeader then
                -- A standalone faction ends the previous subheader's children.
                -- Retain the expansion header, matching the native list indentation.
                if factionData.isChild == false then
                    for j = #headerStack, 2, -1 do headerStack[j] = nil end
                end
                -- Refresh observed hierarchy even for existing metadata; earlier scans
                -- may have retained a subheader after its last genuine child.
                local existing = metadata[factionData.factionID]
                if existing and type(factionData.isChild) == "boolean" then
                    local path = {}
                    for j = 1, #headerStack do path[j] = headerStack[j] end
                    existing.parentHeaders = path
                end
                -- Other metadata is populated only when first encountered.
        ...
        if factionData.factionID and (not factionData.isHeader or factionData.isHeaderWithRep) then
            ...
            if isRenownFaction or (not isRenownFaction and currentValue) then
                ...
                reputations[factionData.factionID] = {
                    isAccountWide = ns.Wealth and ns.Wealth.Public(factionData.isAccountWide) and factionData.isAccountWide == true or nil,
