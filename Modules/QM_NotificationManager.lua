...
local CURRENT_VERSION = "2.0.0-Release"
...
local CHANGELOG = {
    ...
    date = "2026-09-15",
    changes = {
        L["UPDATE_200_INTERFACE"],
        L["UPDATE_200_PROFESSIONS"],
        L["UPDATE_200_SEARCH_WEALTH"],
        L["UPDATE_200_PROGRESSION"],
    }
}
...
function TheQuartermaster:ShowUpdateNotification(changelogData)
    ...
    popup:SetBackdropBorderColor(unpack(borderC))
    ...
    -- Title
    local title = popup:CreateFontString(nil, "OVERLAY", "QuartermasterFontTitle")
    title:SetPoint("TOP", logo, "BOTTOM", 0, -10)
    title:SetText("|cff" .. ToHex(unpack(COLORS.textNormal)) .. "The Quartermaster|r")
    
    -- Version subtitle
    local versionText = popup:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    -- "What's New" label
    local whatsNewLabel = popup:CreateFontString(nil, "OVERLAY", "QuartermasterFontHeading")
    whatsNewLabel:SetPoint("TOP", separator, "BOTTOM", 0, -15)
    whatsNewLabel:SetText("|cff" .. ToHex(unpack(COLORS.textNormal)) .. "What's New|r")
    ...
    if ns.UI_ThemeScrollBar then ns.UI_ThemeScrollBar(scrollFrame) end
    ...
    for i, change in ipairs(changelogData.changes) do
        local bullet = scrollChild:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    closeBtn:SetBackdropColor(unpack(COLORS.bgCard))
    closeBtn:SetBackdropBorderColor(unpack(COLORS.border))
    
    local closeBtnText = closeBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    closeBtnText:SetPoint("CENTER")
    closeBtnText:SetText(L["GOT_IT"])
    closeBtnText:SetTextColor(unpack(COLORS.textNormal))
...
function TheQuartermaster:ShowToastNotification(config)
    ...
    local titleColor = config.titleColor or COLORS.textNormal
    ...
    popup:SetBackdropBorderColor(unpack(borderC)) -- Theme border
    ...
    local message = popup:CreateFontString(nil, "OVERLAY", "QuartermasterFontBody")
    ...
    local closeBtnText = closeBtn:CreateFontString(nil, "OVERLAY", "QuartermasterFontHeading")
    closeBtnText:SetPoint("CENTER")
