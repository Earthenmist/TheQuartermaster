--[[
    The Quartermaster - Cache Manager Module
...
-- ============================================================================
-- PUBLIC API (TheQuartermaster methods)
-- ============================================================================
...
function TheQuartermaster:GetCachedCharacters()
    local cached, hit = GetCache("characters", "list")
    if hit then
        return ns.SortCharacterRows(self.db, cached)
    end
...
function TheQuartermaster:GetCachedPvEData(characterKey)
    local cached, hit = GetCache("pve", characterKey)
    if hit then
        return self:GetPvEView(cached)
    end
    ...
    if self.db.global.characters and self.db.global.characters[characterKey] then
        ...
        if pveData then
            ...
            return self:GetPvEView(pveData)
        end
    end
...
function TheQuartermaster:GetCachedSearchResults(searchTerm)
    local searchKey = searchTerm:lower()
    local cached, hit = GetCache("search", searchKey)
    if hit then
        local active = {}
        for _, row in ipairs(cached) do
            if not row.mailExpiresAt or row.mailExpiresAt > time() then active[#active + 1] = row end
        end
        return active
    end
...
function TheQuartermaster:GetCachedCollectionStats()
...
function TheQuartermaster:GetCachedProfessionData(characterKey)
...
function TheQuartermaster:InvalidateCharacterCache()
...
function TheQuartermaster:InvalidateItemCache()
...
function TheQuartermaster:InvalidatePvECache(characterKey)
...
function TheQuartermaster:InvalidateCollectionCache()
    local playerKey = UnitName("player") .. "-" .. GetRealmName()
...
function TheQuartermaster:InvalidateProfessionCache(characterKey)
    if characterKey then
...
function TheQuartermaster:GetCachedReputationData(characterKey)
...
function TheQuartermaster:InvalidateReputationCache(characterKey)
    if characterKey then
...
function TheQuartermaster:ClearAllCaches()
    ClearAllCaches()
end
...
function TheQuartermaster:GetCacheStats()
...
function TheQuartermaster:PrintCacheStats()
...
local function CleanupExpiredCaches()
    ...
    if removed > 0 and TheQuartermaster then
    end
end

-- Start cleanup timer (every 60 seconds)
if TheQuartermaster then
    TheQuartermaster:ScheduleRepeatingTimer(CleanupExpiredCaches, 60)
end
...
function TheQuartermaster:WarmupCaches()
...
function TheQuartermaster:InitializeCacheInvalidation()
