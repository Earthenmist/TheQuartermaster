--[[
    The Quartermaster - API Wrapper Module
    ...
    Use:        TheQuartermaster:API_GetBagSize(bagID)
]]
...
-- Item info caches (reduce repeated API calls during scans/tooltips)
local itemInfoCache = {}            -- [itemID] = { ... } (GetItemInfo-style returns)
local itemInfoInstantCache = {}     -- [itemID] = { ... } (GetItemInfoInstant-style returns)

local function CacheableItemID(itemID)
    if not itemID then return nil end
    local t = type(itemID)
    if t == "number" then return itemID end
    if t == "string" then
...
function TheQuartermaster:API_GetBagSize(bagID)
...
function TheQuartermaster:API_GetContainerItemID(bagID, slotID)
...
function TheQuartermaster:API_SortBags(bagID)
...
function TheQuartermaster:API_SortBankBags()
...
function TheQuartermaster:API_PickupItem(bagID, slotID)
...
function TheQuartermaster:API_GetFreeBagSlots(bagID)
...
function TheQuartermaster:API_UseItem(bagID, slotID)
    -- CRITICAL: Protected function, check combat
    if InCombatLockdown() then
        if self and self.Print then
...
function TheQuartermaster:API_GetItemInfo(itemID)
    local id = CacheableItemID(itemID)
    ...
    if apiAvailable.item and C_Item.GetItemInfo then
        a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z = C_Item.GetItemInfo(id or itemID)
    elseif GetItemInfo then
        a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z = GetItemInfo(id or itemID)
    end

    -- Only cache if we got a real name back (otherwise item data may not be loaded yet)
    if id and a then
        itemInfoCache[id] = {a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z}
    end
...
function TheQuartermaster:API_GetItemInfoInstant(itemID)
    local id = CacheableItemID(itemID)
    if id and itemInfoInstantCache[id] then
        return unpack(itemInfoInstantCache[id])
    end

    local a,b,c,d,e,f,g,h,i,j
    if apiAvailable.item and C_Item.GetItemInfoInstant then
        a,b,c,d,e,f,g,h,i,j = C_Item.GetItemInfoInstant(id or itemID)
    elseif GetItemInfoInstant then
        a,b,c,d,e,f,g,h,i,j = GetItemInfoInstant(id or itemID)
    end

    if id and a then
        itemInfoInstantCache[id] = {a,b,c,d,e,f,g,h,i,j}
    end
...
function TheQuartermaster:API_GetItemName(itemID)
...
function TheQuartermaster:API_GetItemQuality(itemID)
...
function TheQuartermaster:API_IsBankOpen(bankType)
...
function TheQuartermaster:API_GetNumBankSlots()
...
function TheQuartermaster:API_CanUseBank(bankType)
...
function TheQuartermaster:API_CanDepositMoney()
...
function TheQuartermaster:API_CanWithdrawMoney()
...
function TheQuartermaster:API_AutoBankItem(bagID, slotID)
...
function TheQuartermaster:API_GetMoney()
    if GetMoney then
        return GetMoney()
    end
    return 0
...
function TheQuartermaster:API_FormatMoney(amount)
...
function TheQuartermaster:API_GetWeeklyRewards()
...
function TheQuartermaster:API_GetMythicPlusRuns(includeIncomplete, includePreviousWeeks)
...
function TheQuartermaster:API_GetNumSavedInstances()
...
function TheQuartermaster:API_GetSavedInstanceInfo(index)
...
function TheQuartermaster:API_GetNumMounts()
...
function TheQuartermaster:API_GetNumPets()
...
function TheQuartermaster:API_GetNumToys()
...
function TheQuartermaster:API_GetAchievementPoints()
...
function TheQuartermaster:API_GetServerTime()
...
function TheQuartermaster:API_GetSecondsUntilWeeklyReset()
...
function TheQuartermaster:API_SetTooltipItem(tooltip, itemLink)
...
function TheQuartermaster:API_GetUnitName(unit)
    if UnitName then
...
function TheQuartermaster:API_GetUnitClass(unit)
    if UnitClass then
...
function TheQuartermaster:API_GetUnitLevel(unit)
    if UnitLevel then
...
function TheQuartermaster:API_GetUnitRace(unit)
    if UnitRace then
...
function TheQuartermaster:API_GetUnitFaction(unit)
...
function TheQuartermaster:API_GetRealmName()
...
function TheQuartermaster:API_GetNormalizedRealmName()
...
function TheQuartermaster:InitializeAPIWrapper()
...
function TheQuartermaster:GetAPICompatibilityReport()
    return {
...
function TheQuartermaster:PrintAPIReport()
