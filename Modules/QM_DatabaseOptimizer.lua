--[[
    ...
    Features:
    - Preserve inactive and partially collected character records
    ...
    if self.db.global.warbandBank and self.db.global.warbandBank.items then
        for bagID, bagData in pairs(self.db.global.warbandBank.items) do
            for slotID, item in pairs(bagData) do
                -- Preserve unresolved metadata when either identity form is available.
                if not item.itemID and not item.itemLink then
...
-- Incomplete metadata may belong to a character whose first scan is still pending.
-- Retain the compatibility entry point without deleting those records.
...
function TheQuartermaster:DeleteCharacter(characterKey)
    ...
    -- Remove only this character's domain records; shared banks remain intact.
    if self.RemoveIdentityRecovery then self:RemoveIdentityRecovery(characterKey) end
    for _, domain in ipairs({ "mail", "currencies", "reputation", "statistics" }) do
        local data = self.db.global[domain]
        if data and data.perCharacter then
            data.perCharacter[characterKey] = nil
        end
    end
    if self.db.global.lastScans then
        self.db.global.lastScans[characterKey] = nil
    end
...
function TheQuartermaster:OptimizeDatabase()
    ...
    -- Character retention is independent of age or completeness of metadata.
